...
| [**Title**](AgentSessionClient.md#agentsessionsessionidtitleput) | **PUT** /AgentSession/{sessionId}/title | Renames a session by setting an explicit, user-provided title. Overrides any LLM-generated title and prevents the background title generator from overwriting it (it only fills in a title when one isn&#39;t already set). |
...
<a id="agentsessionsessionidtitleput"></a>
# **Title**
> AgentSessionResponse Title (Guid sessionId, UpdateSessionTitleRequest? updateSessionTitleRequest = null)
...
namespace Example
{
    public class TitleExample
    {
        public static void Main()
        {
            ...
            var apiInstance = new AgentSessionClient(config);
            var sessionId = "sessionId_example";  // Guid | 
            var updateSessionTitleRequest = new UpdateSessionTitleRequest?(); // UpdateSessionTitleRequest? |  (optional) 

            try
            {
                // Renames a session by setting an explicit, user-provided title. Overrides any LLM-generated title and prevents the background title generator from overwriting it (it only fills in a title when one isn't already set).
                AgentSessionResponse result = apiInstance.Title(sessionId, updateSessionTitleRequest);
                Debug.WriteLine(result);
            }
            catch (ApiException  e)
            {
                Debug.Print("Exception when calling AgentSessionClient.Title: " + e.Message);
                Debug.Print("Status Code: " + e.ErrorCode);
                Debug.Print(e.StackTrace);
            }
        }
...
#### Using the TitleWithHttpInfo variant
This returns an ApiResponse object which contains the response data, status code and headers.
...
{
    // Renames a session by setting an explicit, user-provided title. Overrides any LLM-generated title and prevents the background title generator from overwriting it (it only fills in a title when one isn't already set).
    ApiResponse<AgentSessionResponse> response = apiInstance.TitleWithHttpInfo(sessionId, updateSessionTitleRequest);
    Debug.Write("Status Code: " + response.StatusCode);
    Debug.Write("Response Headers: " + response.Headers);
    Debug.Write("Response Body: " + response.Data);
}
...
{
    Debug.Print("Exception when calling AgentSessionClient.TitleWithHttpInfo: " + e.Message);
    Debug.Print("Status Code: " + e.ErrorCode);
    Debug.Print(e.StackTrace);
}
```
...
|------|------|-------------|-------|
| **sessionId** | **Guid** |  |  |
| **updateSessionTitleRequest** | [**UpdateSessionTitleRequest?**](UpdateSessionTitleRequest?.md) |  | [optional]  |
